...
const NominatedCard = ({
    ...
    const updateSelection = async (selection: boolean) => {
        fetch("/api/voting2024/update_vote", {
            method: "POST",
            body: JSON.stringify({
                id: props.id,
                option: selection
...
const AddNomineeForm = () => {
    ...
    const checkNomineeExist = async () => {
        setLoading(true);
        fetch("/api/voting2024/verify_nominees", {
            method: "POST",
            body: JSON.stringify({
                id: nomineeIdInput
            })
    ...
    const submitNominee = async () => {
        ...
        fetch("/api/voting2024/new_nominees", {
            method: "POST",
            body: JSON.stringify({
    ...
    return (
        <div className="bg-whte dark:bg-zinc-900 dark:text-slate-200 rounded-lg p-5 shadow-md w-full max-w-[40rem] border">
            ...
            <form className="flex flex-row items-center space-x-10" onSubmit={(e) => {
                ...
                <button className="bg-black dark:bg-slate-800 text-white w-24 h-8 rounded-sm" type="submit">
                    搜尋
                </button>
            </form>
            ...
            {nomineeExist &&
                <div className="pt-5 flex flex-col">
                    ...
                    <button className="bg-orange-700 hover:bg-orange-800 text-white w-24 h-8 rounded-md self-center" onClick={() => {
                        submitNominee()
                    }}>
                        確認提名
                    </button>
